    ...
    if openai_api_key_from_secrets:
        try:
            ...
            try:
                ...
                OPENAI_AUTH_ERROR_MESSAGE = f"🚨 OpenAI AuthenticationError: Your API Key is invalid, expired, or your account has billing issues. Please check your OpenAI dashboard. Error: {auth_err}"
                st.error(OPENAI_AUTH_ERROR_MESSAGE)
...
def get_ai_sentiment(title, summary, ticker, current_debug_mode):
    ...
    if not use_ai_sentiment or not openai_client:
        return basic_sentiment_analysis(f"{title} {summary}")
    ...
    try:
        ...
        if current_debug_mode:
            st.caption(f"🤖 Attempting OpenAI call for {ticker}: '{title[:30]}...'")
        ...
        else: st.caption(f"AI sentiment for {ticker} failed (General). Using basic.")
        return basic_sentiment_analysis(f"{title} {summary}")
            ...
            try:
                ...
                if current_price and previous_close:
                    ...
                    ticker_prices_cache[ticker_symbol_for_price] = {
                        ...
                        'pre_market_change': ((pre_market_price - previous_close) / previous_close * 100) if pre_market_price and previous_close and previous_close != 0 else None,
                        'post_market_change': ((post_market_price - current_price) / current_price * 100) if post_market_price and current_price and current_price != 0 else None,
        ...
        with st.container(border=True, key=f"news_article_main_v3_{ticker_symbol}_{article_idx}"): 
            ...
            if price_data:
                with meta_col2:
                    ...
                    delta_val = f"{change_dollar:.2f} ({change_pct:.2f}%)"
                    ...
                    try:
                        ...
                        if not hist_chart.empty:
                            ...
                            if news_time_utc and news_time_utc != datetime.min:
                                ...
                                if not hist_chart.empty and hist_chart.index[0] <= news_time_for_chart <= hist_chart.index[-1]:
